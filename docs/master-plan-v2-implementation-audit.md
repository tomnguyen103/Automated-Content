...
| AI influencer generated asset persistence and review handoff | DONE | `lib/media/workflow-adapters.ts`, `app/api/media/generated/[workspaceId]/[jobId]/[asset]/route.ts`, `tests/jobs/media-workflows.test.ts` | Local persistence is in `media_generation_jobs.output`; live storage/ImageKit copy is BLOCKED-EXTERNAL pending real storage/provider setup. |
